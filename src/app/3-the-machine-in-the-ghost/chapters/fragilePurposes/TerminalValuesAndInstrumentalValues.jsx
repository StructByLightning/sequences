...
export default function TerminalValuesAndInstrumentalValues() {
  return <Chapter title={["Terminal Values and", "Instrumental Values"]}>
    ...
    <ul>
      <li>
        <code>Outcomes : type Outcome[]</code>
        <ul>
          <li>list of possible outcomes</li>
          <li>&#123;sister lives, sister dies&#125;</li>
        </ul>
      </li>


      <li>
        <code>Actions : type Action[]</code>
        <ul>
          <li>list of possible actions</li>
          <li>&#123;administer penicillin, don’t administer penicillin &#125;</li>
        </ul>
      </li>

      <li>
        <code>Utility_function : type Outcome ⇒ Utility</code>

        <ul>
          <li>utility function that maps each outcome onto a utility</li>
          <li>(a utility being representable as a real number between negative and positive infinity)</li>
          <li><img src="https://www.readthesequences.com/wiki/uploads/TerminalValuesAndInstrumentalValues_formula_1.svg" height="60px" /></li>
        </ul>
      </li>

      <li>
        <code> Conditional_probability_function : type Action ⇒ (Outcome ⇒ Probability)</code>

        <ul>
          <li>conditional probability function that maps each action onto a probability distribution over outcomes</li>
          <li>(a probability being representable as a real number between 0 and 1)</li>
          <li><img src="https://www.readthesequences.com/wiki/uploads/TerminalValuesAndInstrumentalValues_formula_2.svg" height="150px" /></li>
        </ul>
      </li>
    </ul>
    ...
    <ul>
      <li>
        <code>Expected_Utility : Action A ⇒ (Sum O in Outcomes: Utility(O) * Probability(O|A))</code>
        <ul>
          <li>The “expected utility” of an action equals the sum, over all outcomes, of the utility of that outcome times the conditional probability of that outcome given that action.</li>
          <li><img src="https://www.readthesequences.com/wiki/uploads/TerminalValuesAndInstrumentalValues_formula_3.svg" height="60px" /></li>
        </ul>
      </li>

      <li>
        <code>Choose :</code><br />
        <code>⇒ (Argmax A in Actions: Expected_Utility(A))</code>
        <ul>
          <li>Pick an action whose “expected utility” is maximal.</li>
          <li>&#123; return: administer penicillin&#125;</li>
        </ul>
      </li>
    </ul>
    <p>For every action, calculate the conditional probability of all the consequences that might follow, then add up the utilities of those consequences times their conditional probability. Then pick the best action.</p>
    ...
    <p>So you say,</p>
    <pre><code>TYPE ERROR: No constructor found for</code><br /><code>Expected_Utility ⇒ Utility.</code></pre>
    ...
    <p>The choices of our simple decision system are those with highest Expected_Utility, but this doesn’t say anything whatsoever about <em>where it steers the future</em>. It doesn’t say anything about the utilities the decider assigns, or which real-world outcomes are likely to happen as a result. It doesn’t say anything about the mind’s function as an engine.</p>
    <p>The physical cause of a physical action is a cognitive state, in our ideal decider an Expected_Utility, and this expected utility is calculated by evaluating a utility function over imagined consequences. To save your son’s life, you must imagine the event of your son’s life being saved, and this imagination is not the event itself. It’s a <em>quotation</em>, like the difference between “snow” and snow. But that doesn’t mean that what’s <em>inside the quote marks</em> must itself be a cognitive state. If you choose the action that leads to the future that you represent with “my son is still alive,” then you have functioned as an engine to steer the future into a region where your son is still alive. Not an engine that steers the future into a region where <em>you represent the sentence</em> “my son is still alive.” To steer the future there, your utility function would have to return a high utility when fed “ “my son is still alive” ”, the quotation of the quotation, your imagination of yourself imagining. Recipes make poor cake when you grind them up and toss them in the batter.</p>
    ...
    <p>And then the instrumental values would start coming back. If the causal network was sufficiently regular, you could find a state <em>B</em> that tended to lead to <em>C</em> regardless of how you achieved <em>B</em>. Then if you wanted to achieve <em>C</em> for some reason, you could plan efficiently by first working out a <em>B</em> that led to <em>C</em>, and then an <em>A</em> that led to <em>B</em>. This would be the phenomenon of “instrumental value”—<em>B</em> would have “instrumental value” because it led to <em>C</em>. The state <em>C</em> itself might be terminally valued—a term in the utility function over the total outcome. Or <em>C</em> might just be an instrumental value, a node that was not directly valued by the utility function.</p>
    ...
    <p>This important distinction often gets <em>flushed down the toilet</em> in angry arguments. People with factual disagreements and shared values each decide that their debating opponents must be sociopaths. As if your hated enemy, gun control/rights advocates, <em>really wanted to kill people</em>, which should be implausible as realistic psychology.</p>
